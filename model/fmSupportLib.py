...
def fmDemodArctan(I, Q, block_count = 0, buf_I = 0.0, buf_Q = 0.0):
	#
	...
	for k in range(len(I)):
		...
		if (k != 0):
			...
			if (k == (len(I) - 1)):
				buf_I = I[k]
				buf_Q = Q[k]
		elif ((block_count == 0) & (k==0)):
			fm_demod[k] = 0.0
		elif ((block_count != 0) & (k==0)):
			fm_demod[k] = (I[k] * (Q[k]-buf_Q) - Q[k] * (I[k]-buf_I))/(I[k]**2 + Q[k]**2)
		...
		[fm_demod[k-1], fm_demod[k]] = np.unwrap([fm_demod[k-1], fm_demod[k]])
		# if (block_count == 0):
		# 	print(fm_demod[k])
	#
...
def estimatePSD(samples, NFFT, Fs):
	...
	# for i in range (0,10):
	# 	print("This is psd_seg[",i,"]: ", psd_seg[i], "\n")
