...
def fmPll(pllIn, freq, Fs, state,\
		ncoScale = 0.5, phaseAdjust = 0.0, normBandwidth = 0.005):
	...
	ncoOut_Q = np.zeros(len(pllIn)+1)
	ncoOut_I = np.zeros(len(pllIn)+1)
	ncoOut_Q[0] = state["ncoOut_Q_buf"]
	ncoOut_I[0] = state["ncoOut_I_buf"]
	...
	state["ncoOut_I_buf"] = ncoOut_Q[-1]
	state["ncoOut_Q_buf"] = ncoOut_I[-1]
	# for stereo only the in-phase NCO component should be returned
	return ncoOut_Q, ncoOut_I, state
